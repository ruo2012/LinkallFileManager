...
namespace FileManager.DAL
{
    ...
    /// 数据访问类:文件版本
    /// </summary>
    public partial class FileVersionDal
    {
        private string databaseprefix; //数据库表名前缀
        public FileVersionDal(string _databaseprefix)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public FileVersion GetModel(int id)
        {
            StringBuilder strSql = new StringBuilder();
        ...
        /// 根据用户名返回上一次登录记录
        /// </summary>
        public FileVersion GetModel(string user_name, int top_num, string action_type)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 id from (select top " + top_num + " id from " + databaseprefix + "FileVersion where user_name=@user_name and action_type=@action_type order by id desc) as T ");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "FileVersion ");
