...
namespace FileManager.DAL
{
    ...
    /// 数据访问类:文件操作日志
    /// </summary>
    public partial class FileLogDal
    {
        private string databaseprefix; //数据库表名前缀
        public FileLogDal(string _databaseprefix)
        {
        ...
        public bool Exists(int id)
        {
            ...
            strSql.Append("select count(1) from " + databaseprefix + "FileLog");
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) as H from " + databaseprefix + "FileLog ");
            if (strWhere.Trim() != "")
            {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into " + databaseprefix + "FileLog(");
            strSql.Append("[UserID],[UserName],[UserRealName],[ActionType],ActionName,[Remark],[Ip],[ProjectID],[ProjectName],[ProjectPath],[ClientPath],[FileId],[FileName],[FileVerID],[FileVer],[FileType],[ParentForderId],[ActionCode],[AddTime])");
            ...
            SqlParameter[] parameters = {
					...
					new SqlParameter("@FileVerID", SqlDbType.Int,4),
					new SqlParameter("@FileVer", SqlDbType.Int,4),
					new SqlParameter("@FileType", SqlDbType.Int,4),
					new SqlParameter("@ParentForderId", SqlDbType.Int,4),
            ...
            parameters[12].Value = model.FileName;
            parameters[13].Value = model.FileVerID;
            parameters[14].Value = model.FileVer;
            parameters[15].Value = model.FileType;
            parameters[16].Value = model.ParentForderId;
            parameters[17].Value = model.ActionCode;

            parameters[18].Value = model.AddTime;
            parameters[19].Value = ParameterDirection.Output;

            List<CommandInfo> sqllist = new List<CommandInfo>();
            CommandInfo cmd = new CommandInfo(strSql.ToString(), parameters);
            sqllist.Add(cmd);
            DbHelperSQL.ExecuteSqlTranWithIndentity(sqllist);
            int obj =(int)parameters[19].Value;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("delete from " + databaseprefix + "FileLog ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 * from " + databaseprefix + "FileLog ");
            strSql.Append(" where id=@id");
            SqlParameter[] parameters = {
					new SqlParameter("@id", SqlDbType.Int,4)};
            parameters[0].Value = id;

            Model.FileLogModel model = new Model.FileLogModel();
        ...
        public Model.FileLogModel GetModel(string actionCode)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select  top 1 * from " + databaseprefix + "FileLog ");
            strSql.Append(" where actionCode=@actionCode");
            SqlParameter[] parameters = {
					new SqlParameter("@actionCode", SqlDbType.NVarChar)};
        ...
        public List<string> GetLogIps(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append("  IP ");
            strSql.Append(" FROM " + databaseprefix + "FileLog ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        public DataSet GetAllInfoList(int Top, string strWhere, string filedOrder)
        {
            ...
            strSql.Append(" fl.*,u.id AS file_user_id,u.UserName as file_user_name,u.realName as file_user_realname ");
            //strSql.Append(" FROM " + databaseprefix + "FileLog ");
            strSql.Append("  from FM_FileLog fl LEFT JOIN FM_File f  ON   fl.FileID =f.id LEFT JOIN  FM_USER u ON f.Update_UserName = u.UserName   ");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
        ...
        {
            List<Model.FileLogModel> modelList = new List<Model.FileLogModel>();
        ...
        /// <returns></returns>
        private Model.FileLogModel GetModelByDataRow(DataRow item)
        {
            Model.FileLogModel model = new Model.FileLogModel();
            ...
            model.FileID = int.Parse(item["FileID"].ToString());
            model.FileName = item["FileName"].ToString();
            model.FileVerID = int.Parse(item["FileVerID"].ToString());
            model.FileVer = int.Parse(item["FileVer"].ToString());
            model.ActionName = item["ActionName"].ToString();
            model.UserRealName = item["UserRealName"].ToString();
            model.FileType = int.Parse(item["FileType"].ToString());
            ...
            model.ParentForderId = int.Parse(item["ParentForderId"].ToString());
        ...
        public Model.FileLogModel GetModel(string user_name, int top_num)
        {
            ...
            if (rows > 1)
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 id from (select top " + top_num + " id from " + databaseprefix
                    + "FileLog where UserName=@user_name  order by id desc) as T ");
                strSql.Append(" order by id asc");
                SqlParameter[] parameters = {
					new SqlParameter("@user_name", SqlDbType.NVarChar,100) };
                parameters[0].Value = user_name;
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM " + databaseprefix + "FileLog");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }
